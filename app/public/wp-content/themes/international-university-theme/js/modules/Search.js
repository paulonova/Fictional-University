...
class Search {
        ...
        <div class="row">
          ...
          <div class="one-third">
            ...
            <h2 class="search-overlay__section-title">Professors</h2>
              ${results.professors.length ? `<ul class="professor-cards">` : `<p> No professors matches this search.</p>`}
          ...
          <div class="one-third">
            <h2 class="search-overlay__section-title">Campuses</h2>
              ${results.campuses.length ? `<ul class="min-list link-list">` : `<p>No campuses matches this search
                <a href="${universityData.root_url}/campuses">View all campuses</a></p>`}
              ${results.campuses.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join('')}
              ${results.campuses.lenght ? '</ul>' : ''}

            <h2 class="search-overlay__section-title">Events</h2>
              ${results.events.length ? `` : `<p>No events matches this search
  ...
  openOverlay(){
    ...
    return false;  // will prevent the default behaveur of <a></a> element
  }
